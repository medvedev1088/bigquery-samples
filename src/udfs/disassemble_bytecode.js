var hexToIntArray = function (hexString) {
...
opcodes = function (op, full) {
  var code = codesMap[op] ? codesMap[op] : ['INVALID', 0, 0, 0, false, false];
  var opcode = code[0];
...
var parseCode = function (raw) {
  ...
  for (var i = 0; i < raw.length; i++) {
    ...
    if (opcode.name.slice(0, 4) === 'PUSH') {
      ...
      opcode.pushData = hexConvert(opcode.pushData);
      i += length
    }
    code.push(opcode)
  }
